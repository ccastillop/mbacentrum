...
describe LevelsController do
  ...
  def mock_level(stubs={})
    @mock_level ||= mock_model(Level, stubs).as_null_object
  end

  describe "GET index" do
    it "assigns all levels as @levels" do
      Level.stub(:all) { [mock_level] }
      get :index
      assigns(:levels).should eq([mock_level])
    end
  end

  describe "GET show" do
    it "assigns the requested level as @level" do
      Level.stub(:find).with("37") { mock_level }
      get :show, :id => "37"
      assigns(:level).should be(mock_level)
    end
  end

  describe "GET new" do
    it "assigns a new level as @level" do
      Level.stub(:new) { mock_level }
      get :new
      assigns(:level).should be(mock_level)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created level as @level" do
        Level.stub(:new).with({'these' => 'params'}) { mock_level(:save => true) }
        post :create, :level => {'these' => 'params'}
        assigns(:level).should be(mock_level)
      end

      it "redirects to the created level" do
        Level.stub(:new) { mock_level(:save => true) }
        post :create, :level => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved level as @level" do
        Level.stub(:new).with({'these' => 'params'}) { mock_level(:save => false) }
        post :create, :level => {'these' => 'params'}
        assigns(:level).should be(mock_level)
      end

      it "re-renders the 'new' template" do
        Level.stub(:new) { mock_level(:save => false) }
        post :create, :level => {}
  ...
  describe "PUT update" do
    ...
    describe "with invalid params" do
      it "assigns the level as @level" do
        Level.stub(:find) { mock_level(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:level).should be(mock_level)
      end
